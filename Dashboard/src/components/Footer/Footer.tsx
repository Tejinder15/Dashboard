...
const Footer = () => {
  return (
    <div>
      <footer className="flex items-center justify-between max-w-custom mx-auto border-t-[0.5px] border-white py-5 px-8">
        ...
        <ul className="flex items-center justify-between w-1/3 ">
          {navLinks.footer.primary.map((item, index) => (
            <li key={index} className="text-xs font-poppins text-white">
              {item.name}
            </li>
          ))}
        </ul>
